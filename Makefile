...
help:
	...
	@echo "  run <folder_name>  Runs Espada on the folder with the given name."
...
#Defines a target named run. This target will run Espada on the folder with the given name.
run:
	@echo -e "$(COLOR_CYAN)Running Espada on $(COLOR_GREEN)$(name)$(COLOR_CYAN) folder...$(COLOR_RESET)" && \
	poetry run espada projects/$(name)
