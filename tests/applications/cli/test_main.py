...
import espada.applications.cli.main as main

from espada.applications.cli.main import load_prompt
...
class TestMain:
    #  Runs espada cli interface for many parameter configurations, BUT DOES NOT CODEGEN! Only testing cli.
    ...
    #  Runs espada with improve mode and improves an existing project in the specified path.
    ...
    #  Runs espada with improve mode and improves an existing project in the specified path, with skip_file_selection
    ...
    def test_improve_existing_project_diff_timeout(self, tmp_path, monkeypatch):
        ...
        #             """
        # )
        # os.environ["ESPADA_TEST_MODE"] = "True"
        # simplified_main(str(p), "improve")
        # DiskExecutionEnv(path=p)
        # del os.environ["ESPADA_TEST_MODE"]

    #  Runs espada with lite mode and generates a project with only the main prompt.
    ...
    #  Runs espada with clarify mode and generates a project after discussing the specification with the AI.
    ...
    #  Runs espada with self-heal mode and generates a project after discussing the specification with the AI and self-healing the code.
...
class TestLoadPrompt:
    ...
    def test_load_prompt_no_file_improve_mode_false(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch(
                "builtins.input",
                return_value="What application do you want espada to generate?",
            ):
                result = load_prompt(
            ...
            assert isinstance(result, Prompt)
            assert (
                result.text == "What application do you want espada to generate?"
            )
            assert result.image_urls is None
...
#         DiskExecutionEnv(path=p)
#         assert (
#             (p / f".espada/memory/{DEBUG_LOG_FILE}").read_text().strip()
#             == """UPLOADED FILES:
# ```
# File: main.py
...
#         monkeypatch.setattr("builtins.input", lambda _: next(gen))
#         monkeypatch.setattr(
#             "espada.core.default.steps.salvage_correct_hunks",
